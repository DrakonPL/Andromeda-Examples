...
void TestLight4::Init()
{
	...
	//load shader
	_shader = _shaderManager->LoadFromFile("model", "Assets/Shaders/lightening4_texture", "Assets/Shaders/lightening4_texture", TextureNormal);
	...
	//set cube color
	_cube->SetColor(glm::vec4(1.0f, 1.0f, 1.0f,1.0f));
...
void TestLight4::Enter()
{

}

void TestLight4::CleanUp()
{
	delete _cube;
	delete _lamp;
	delete _cam;
...
void TestLight4::Resume()
{

}

void TestLight4::GamePause()
{

}

void TestLight4::GameResume()
{

}

void TestLight4::HandleEvents(GameManager* manager)
{
	...
	if (_keyboard != 0)
	{
		...
		//check if exit
		if (_keyboard->KeyDown(Key::Escape))
			manager->Quit();
	}	
	...
	if (_gamepad != 0)
	{
		...
		//check if exit
		if (_gamepad->KeyDown(Gamepad::Start))
			manager->Quit();
	}
...
void TestLight4::Update(GameManager* manager)
{
	_dt = _timer->GetDelta();
	_rotation += _dt;
...
void TestLight4::Draw(GameManager* manager)
{
	//start frame
	...
	//draw cube
	{
		...
		glm::vec3 lampPosition = _lamp->GetPosition();
		...
		_cube->GetShader()->SetUniform(FragmentShader, "lightPos", lampPosition);
		_cube->GetShader()->SetUniform(FragmentShader, "viewPos", viewPosition);
