...
public:
	FindGroundAndSteps(const SimpleCharacterController *controller,
...
private:
	...
	const SimpleCharacterController *mController;
...
class ResolveStepUp {
public:
	ResolveStepUp(const SimpleCharacterController *controller,
...
SimpleCharacterController::SimpleCharacterController(btRigidBody *body,
...
void SimpleCharacterController::setupBody()
{
	assert(mRigidBody);
...
void SimpleCharacterController::updateAction(btCollisionWorld *collisionWorld,
...
void SimpleCharacterController::updateVelocity(float dt)
{
...
void SimpleCharacterController::stepUp(float dt)
{
	btTransform transform;
	mRigidBody->getMotionState()->getWorldTransform(transform);
...
void SimpleCharacterController::debugDraw(btIDebugDraw *debugDrawer)
{
	if (mStepping) {
...
void SimpleCharacterController::setMovementDirection(
...
const btVector3 &SimpleCharacterController::getMovementDirection() const
{
	return mMoveDirection;
}

void SimpleCharacterController::resetStatus()
{
...
bool SimpleCharacterController::canJump() const
{
	return mOnGround;
}

void SimpleCharacterController::jump(const btVector3 &dir)
{
	if (!canJump()) {
		return;
	}
...
const btRigidBody *SimpleCharacterController::getBody() const
{
	return mRigidBody;
}

void SimpleCharacterController::cancelStep()
{
	if (mRigidBody) {
		if (mStepping) {
...
FindGroundAndSteps::FindGroundAndSteps(
	const SimpleCharacterController *controller, const btCollisionWorld *world)
{
...
ResolveStepUp::ResolveStepUp(const SimpleCharacterController *controller,
	const btCollisionWorld *world, const btManifoldPoint &cp) :
